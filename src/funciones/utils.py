...
def paginar_tabla(items, encabezado, titulo="", items_por_pagina=10):
    ...
    def mostrar_pagina(pagina):
        ...
        tabla = Table(title=f"[yellow]{titulo}[/yellow]", style="yellow", show_lines=True)
        ...
        console.clear()
        console.print("\n", Panel(f"✅ Resultados encontrados:", style="bold green", expand=False))
        console.print(tabla)
        console.print(f"Página ({pagina}/{total_paginas})")
        console.print("\nEscriba 's' para siguiente | 'a' para anterior | 'e' para salir", style="bold yellow")
